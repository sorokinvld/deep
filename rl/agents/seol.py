...
class AbstractDQNAgent(Agent):
    ...
    def compute_batch_q_values(self, state_batch):
        ...
        state = tf.convert_to_tensor(batch, dtype=tf.float32)
    ...
    def compute_q_values(self, state):
        ...
        q_values = tf.squeeze(q_values, axis=0) 
...
class DQNAgent(AbstractDQNAgent):
    ...
    def __init__(self, model, policy=None, test_policy=None, enable_double_dqn=False, enable_dueling_network=False,
        ...
        if len(output_shape) != 2 or output_shape[1] != self.nb_actions:
            raise ValueError(f'Model output shape {output_shape} is invalid. DQN expects a model that has one dimension for each action, in this case {self.nb_actions}.')
    ...
    def backward(self, reward, terminal):
        ...
        if self.step > self.nb_steps_warmup and self.step % self.train_interval == 0:
            ...
            if self.enable_double_dqn:
                ...
                state = tf.convert_to_tensor(state1_batch, dtype=tf.float32)
                target_q_values = self.target_model.predict_on_batch(state)
                # batch = self.process_state_batch(state_batch)
                # state = tf.convert_to_tensor(batch, dtype=tf.float32)
                # q_values = self.model.predict_on_batch(state)
                # assert q_values.shape == (len(state_batch), self.nb_actions)
            ...
            dope = tf.convert_to_tensor(state0_batch, dtype=tf.float32)
            ins = [dope] if type(self.model.input) is not list else dope
